...
bool ActiveDisplayFollower_Process(ActiveDisplayFollower *f,const ActiveDisplayFollowerInput *i,ActiveDisplayFollowerOutput *o){int32_t d;if(!f||!i||!o)return false;
 ...
 if(d==0){f->candidate_count=i->desired_count;f->candidate_start_ms=i->now_ms;f->confirmation_count=0;f->flags&=(uint8_t)~FLAG_CATCHING_UP;}
 else if(f->flags&FLAG_CATCHING_UP){f->candidate_count=i->desired_count;if(i->now_ms!=f->candidate_start_ms){f->candidate_start_ms=i->now_ms;f->display_count+=d>0?1:-1;}f->confirmation_count=0;if(f->display_count==i->desired_count)f->flags&=(uint8_t)~FLAG_CATCHING_UP;}
