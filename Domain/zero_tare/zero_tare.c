                   ...
                   bool restore_tare)
{
    ...
    if (restore_tare)
    {
        state->tare_weight = restored_tare;
    ...
    bool stable, bool calibration_valid)
{
    ...
    {
        return WEIGHT_ACTION_CALIBRATION_INVALID;
    }
    if (state->tare_active)
    {
        return WEIGHT_ACTION_TARE_ACTIVE;
    }
    if (!stable)
    {
        return WEIGHT_ACTION_NOT_STABLE;
    }
    if (!WeightMath_AbsInt32(current_gross_unrounded, &magnitude) ||
        (magnitude > zero_range))
    {
    ...
    bool calibration_valid, bool overload)
{
    ...
    if (overload)
    {
        return WEIGHT_ACTION_OVERLOAD;
    }
    if (!stable)
    {
        return WEIGHT_ACTION_NOT_STABLE;
    }
    state->tare_weight = current_gross_unrounded;
    state->tare_active = true;
    ++state->tare_sequence;
    return WEIGHT_ACTION_OK;
}
