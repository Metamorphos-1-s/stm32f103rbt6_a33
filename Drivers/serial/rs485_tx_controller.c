...
static uint32_t s_state_enter_cycles;
static uint32_t s_tx_start_cycles;
static bool s_completed;
static Rs485TxErrorReason s_last_error;

static bool ElapsedUs(uint32_t now_cycles, uint32_t start_cycles,
                      uint32_t interval_us)
{
    uint32_t interval_cycles;
    return BSP_TimeUsToCycles(interval_us, &interval_cycles) &&
        BSP_TimeCyclesElapsed(now_cycles, start_cycles, interval_cycles);
}
...
void Rs485TxController_Init(void)
{
    ...
    s_completed = false;
    s_last_error = RS485_TX_ERROR_NONE;
}
...
{
    uint32_t now_cycles = BSP_TimeNowCycles();
    if ((s_state >= RS485_TX_DMA_ACTIVE) &&
        (s_state <= RS485_TX_WAIT_DE_HOLD) &&
        ElapsedUs(now_cycles, s_tx_start_cycles, RS485_TX_TIMEOUT_US))
    {
        Rs485TxController_Abort();
        s_last_error = RS485_TX_ERROR_TIMEOUT;
    ...
    switch (s_state)
    {
        case RS485_TX_ASSERT_DE:
            ...
            s_state_enter_cycles = now_cycles;
            s_tx_start_cycles = now_cycles;
            s_state = RS485_TX_WAIT_DE_SETUP;
            break;
        case RS485_TX_WAIT_DE_SETUP:
            if (ElapsedUs(now_cycles, s_state_enter_cycles,
                          RS485_DE_SETUP_US))
            {
                ...
                {
                    BSP_Rs485SetTransmit(false);
                    s_last_error = RS485_TX_ERROR_DMA_START;
                    s_state = RS485_TX_ERROR;
                }
            }
            break;
...
Rs485TxErrorReason Rs485TxController_GetLastError(void) { return s_last_error; }
