...
int main(void)
{
    ...
    CHECK(GuardedCheckweigh_Process(&guarded, &input, 1951U, &output));
    ...
    input.sample_sequence = 20U; input.sample_timestamp_ms = 2000U;
    CHECK(GuardedCheckweigh_Process(&guarded, &input, 2000U, &output));
    CHECK(output.state == CHECKWEIGH_HIGH);
    input.dynamic_class = CHECKWEIGH_SHADOW_OK;
    input.sample_sequence = 21U; input.sample_timestamp_ms = 2100U;
    CHECK(GuardedCheckweigh_Process(&guarded, &input, 2100U, &output));
