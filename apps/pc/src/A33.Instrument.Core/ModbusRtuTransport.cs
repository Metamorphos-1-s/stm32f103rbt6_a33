...
public sealed class ModbusRtuTransport : IModbusTransport
{
    ...
    { serialPort=new SerialPort(portName,baudRate,parity,dataBits,stopBits){ReadTimeout=50,WriteTimeout=1000};ownsStream=true;interFrameDelay=TimeSpan.FromMilliseconds(Math.Max(1,Math.Ceiling(38500d/baudRate)));Endpoint=$"{portName} {baudRate} {dataBits}{parity.ToString()[0]}{(stopBits==StopBits.One?1:2)}"; }
    ...
    {
        if(serialPort is not null)return await Task.Run(()=>ExchangeSerial(unitId,pdu.ToArray(),cancellationToken),cancellationToken);
        ...
        if(function==(pdu.Span[0]|0x80))remainderLength=2;else if(function==3)remainderLength=prefix[2]+2;else if(function is 6 or 16)remainderLength=5;else remainderLength=2;
    ...
    private ModbusExchangeResult ExchangeSerial(byte unitId,byte[] pdu,CancellationToken token)
    {
        if(serialPort is null||!serialPort.IsOpen)throw new InvalidOperationException("RTU transport is closed.");if(interFrameDelay>TimeSpan.Zero)Thread.Sleep(interFrameDelay);token.ThrowIfCancellationRequested();serialPort.DiscardInBuffer();var request=ModbusRtuCodec.Encode(unitId,pdu);serialPort.Write(request,0,request.Length);
        var prefix=new byte[3];ReadExactlySerial(prefix,token);var function=prefix[1];var remainderLength=function==(pdu[0]|0x80)?2:function==3?prefix[2]+2:function is 6 or 16?5:2;if(prefix.Length+remainderLength>ModbusRtuCodec.MaximumAduLength)throw new ModbusFrameException(ModbusFrameError.Length,"RTU response exceeds 256 bytes.");var remainder=new byte[remainderLength];ReadExactlySerial(remainder,token);var response=prefix.Concat(remainder).ToArray();var decoded=ModbusRtuCodec.Decode(response,unitId,pdu[0]);return new ModbusExchangeResult(decoded,request,response);
    }
    private void ReadExactlySerial(byte[] destination,CancellationToken token){if(serialPort is null)throw new InvalidOperationException("RTU transport is closed.");var offset=0;while(offset<destination.Length){token.ThrowIfCancellationRequested();try{offset+=serialPort.Read(destination,offset,destination.Length-offset);}catch(TimeoutException){}}}
